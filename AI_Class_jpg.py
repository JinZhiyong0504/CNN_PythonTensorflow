...
#            MSE = self.sess.run(self.prediction, feed_dict={self.xs:self.Batch_Train_Setx, self.ys:self.Batch_Train_Sety, self.keep_prob:1})
            ...
            self.prediction1 = self.sess.run(self.prediction,feed_dict={self.xs:self.Batch_Train_Setx, self.ys:self.Batch_Train_Sety, self.keep_prob:1})
...
# initializing
IMI = TensorflowCNNImageClassification(256,256,4,40479)
#set  TrainingSet Directory
IMI.Set_TrainSet_Directory(r'D:\Kaggle\Understanding the Amazon from Space\Split')
#set Image Files Directory
IMI.Set_Image_Directory(r'D:\Kaggle\Understanding the Amazon from Space\train-tif-v2')
# Read Training Sets
IMI.Read_Train_sets(r'D:\Kaggle\Understanding the Amazon from Space\Split','haze.csv',['image_name'],['haze'])

# Define Conv1 layer
IMI.Input_Output_Define([None, 256, 256,4],[None, 1])#256X256X4

IMI.SetConv_Stride([1,1,1,1])

IMI.SetMaxPool([1,16,16,1],[1,4,4,1])
# Define Conv1 layer
IMI.SetConv1([2,2,4,32]) # 64X64X32

IMI.SetConv2([4, 4, 32, 32]) #16X16X32
# define Function1 layer
IMI.SetFun1([32*16*16, 16])
# define Function2 Layer
IMI.SetFun2([16, 1])
#


IMI.CommitteTraining(0.1,run_times = 1,saveFileName = r"D:\Kaggle\Understanding the Amazon from Space\Split\haze")

#IMI.Predict(r"D:\Kaggle\Understanding the Amazon from Space\test-tif-v2",r"D:\Kaggle\Understanding the Amazon from Space\Split\haze")
