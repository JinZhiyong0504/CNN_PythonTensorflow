...
#         x_image = tf.reshape(self.Batch_Train_Setx, [-1, xpixls*Image_layer*ypixls, 1])
    ...
    def Conv1Layer(self,W_conv1_matrix = []):
        ...
        self.h_conv1 = tf.nn.relu(self.conv2d(self.xs, self.W_conv1) + self.b_conv1) # output size 128X128X32
        self.h_conv1_size = []
        self.h_pool1 = self.max_pool(self.h_conv1)   # output size 32X32x32
        self.h_pool1_size = []
        
    def Conv2Layer(self):
        self.W_conv2 = self.weight_variable([4, 4, 32, 128]) # patch 2x2, in size 32, out size 128
        self.b_conv2 = self.bias_variable([128])
        self.h_conv2 = tf.nn.relu(self.conv2d(self.h_pool1, self.W_conv2) + self.b_conv2) # output size 16x16x128
        self.h_pool2 = self.max_pool(self.h_conv2)   # output size 4x4x128
        self.h_pool2_size = []
        self.h_conv2_size = []
    ...
    def Function1Layer(self):
        self.W_fc1 = self.weight_variable([4*4*128, 512])
        self.b_fc1 = self.bias_variable([512])        
#        h_pool2_flat = tf.reshape(self.h_pool2, [-1,64*64*128])
        self.h_fc1 = tf.nn.relu(tf.matmul(tf.reshape(self.h_pool2, [-1,4*4*128]), self.W_fc1) + self.b_fc1)        
...
#                self.Batch_Train_Sety = primary
                self.sess.run(train_step, feed_dict={self.xs:self.Batch_Train_Setx, self.ys:self.Batch_Train_Sety, self.keep_prob:1})
                print("5")                
