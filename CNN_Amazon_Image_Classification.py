...
class TensorflowCNNImageClassification(ArtificialIntelligence):
    def __init__(self,xpixls,ypixls,layers,N_TrainSet,Type = ".tif"):
        ...
        ## define first Convlution parameters patch, stride
        
        ## define the second convlustion parameters, patch, stride
    ...
    def GetBatchxy(self,Batch_len):
        ...
        if self.Batch_Current_Index < self.MaxTrainLen-Batch_len:
            ...
            for i in range(self.Batch_Current_Index, self.Batch_Current_Index+Batch_len):
                ...
                self.Batch_Train_Sety[i-self.Batch_Current_Index,:] = self.trainSet.iloc[i,self.TrainSet_Columns_Begin:(self.TrainSet_Columns_End+1)]
...
#         x_image = tf.reshape(self.Batch_Train_Setx, [-1, xpixls*Image_layer*ypixls, 1])
    ...
    def Input_Output_Define(self,Input_dimension = 0, Output_dimension = 0):
        ...
        self.ys = tf.placeholder(tf.float16, [None, 17])
    ...
    def compute_accuracy(self,v_xs, v_ys):
        y_pre = self.sess.run(self.prediction, feed_dict={xs: v_xs, keep_prob: 1})
        correct_self.prediction = tf.equal(tf.argmax(y_pre,1), tf.argmax(v_ys,1))
        accuracy = tf.reduce_mean(tf.cast(correct_self.prediction, tf.float16))
        result = self.sess.run(accuracy, feed_dict={xs: v_xs, ys: v_ys, keep_prob: 1})
        return result    
...
#        h_pool2_flat = tf.reshape(self.h_pool2, [-1,64*64*128])
    ...
    def Function2Layer(self):
        self.W_fc2 = self.weight_variable([512, 17])
        self.b_fc2 = self.bias_variable([17])
        self.prediction = tf.nn.softmax(tf.matmul(self.h_fc1_drop, self.W_fc2) + self.b_fc2)        
        
    def CommitteTraining(self,steps):
        ##self.GetBatchxy(100) 
        self.Input_Output_Define(256*256*4,17)
        self.Conv1Layer()
        self.Conv2Layer()
        self.Function1Layer()
        self.Function2Layer()
        ...
        cross_entropy = tf.reduce_mean(-tf.reduce_sum(self.ys * tf.log(self.prediction),reduction_indices=[1]))       # loss
        print("2")
        train_step = tf.train.AdamOptimizer(steps).minimize(cross_entropy)
        ...
        with tf.Session() as sess:
            ...
            for _ in range(100):
                ...
                sess.run(train_step, feed_dict={self.xs:self.Batch_Train_Setx, self.ys:self.Batch_Train_Sety, self.keep_prob:0.5})
                print("5")
                
                #if i % 50 == 0:
#                print(compute_accuracy(self.DataSet.test.images, self.DataSet.test.labels))
...
ImageInstance.Read_Train_sets(r'D:\Kaggle\Understanding the Amazon from Space','One_hot_TraingSet.csv',1,17,18)

#ImageInstance.Input_Output_Define(256*256*4,17)
...
ImageInstance.CommitteTraining(0.1)

##ImageInstance.Read_Image_sets(r'D:\Kaggle\Understanding the Amazon from Space\train-jpg')
## read data in 
##ImageInstance.Read_data_sets("D:\Kaggle\Understanding the Amazon from Space\train-jpg")
# first conv layer patch size, output,first pooling size
# Second pooling patch size, output, second pooling size
# function 1 output
##ImageInstance.SetNuralNet() 
# set drop_out_prob, self.patch
##ImageInstance.SetCaculationParameters()
## perform training
##ImageInstance.CommitteTraining()
